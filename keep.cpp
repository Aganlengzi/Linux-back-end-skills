...
free	//查看内存使用量和交换区使用量
df -h	//各分区使用情况
top		//当前各进程运行情况  类似于windows下的任务管理器
...
它是最快的IPC方式，它允许多个不相关的进程去访问同一部分逻辑内存。
它不需要在内核和用户空间中来回拷贝数据，进程访问共享的内存就像是访问自己独占的内存一样。
但是使用者需要提供不同的方式保证同步和互斥。
共享内存的一般过程是：
通信间进程把自己的一段地址空间映射到操作系统提供的一块内存（可以通过ipcs查看操作系统中的共享内存），
在进程的地址空间中，这段地址空间的位置应该是栈下面紧靠近栈的地方
这些进程可以像访问自己进程的地址空间一样访问这块内存（不知道这块内存还被其他进程映射和访问，所以需要使用各种手段保证同步）
可以使用ipcs -m查看系统中目前所有的共享内存，nattch字段显示的是这个共享内存segment和多少个进程关联
然后用ipcs -m -i (shmid) 查看某一个共享内存segment的详细的信息
比如说：
CPID 		共享内存项的[创建程序]的进程标识。
LPID 		连接或者拆离共享内存段的最后一个进程的标识。
ATT_TIME 	最后一次与关联的共享内存段完成连接的时间。
DET_TIME 	最后一次与关联的共享内存段完成拆离的时间。
CHANGE_TIME	创建和改变相关项的时间。
...
[全局(静态)区]存放全局变量和静态变量，初始化的全局变量和静态变量在一块区域中；
[未初始化BSS区]未初始化的全局变量和静态变量在相邻的另一块区域（BSS Block Started by Symbol），在程序执行前会自动清0
[堆区heap：]由低地址向高地址扩展
...
10、如何定位内存泄露？
内存泄露
指由于错误或者不完备的代码造成一些声明的对象实例长期占用内存空间，不能回收。
造成的原因：
new/delete 和malloc/free使用的时候，由于程序员的原因没有释放申请的堆空间。
持续不断申请内存，造成溢出

通过测试发现memory leak
因为微小的内存泄露不容易发现并且也不会造成很大的问题，所以通过让程序运行达到一定的次数。
代码review
特殊的临界值测试
利用一些工具进行测试

http://blog.csdn.net/bzhxuexi/article/details/17397023
11、动态链接和静态链接的区别
链接是要做什么的？
...
软件更新：依赖静态链接库的程序在更新时要重新链接，依赖动态链接库的程序在更新时只需要更新库就可以了
...
当进程检查出收到了一个子进程终止的信号时，缺省情况下，该进程就像没有收到该信号似的，
...
调用fork，然后父进程退出，此时第二个子进程不会是会话的首进程了，这样可以有效防止其再次获得终端
...
更详细的说，就是内核会将暂时不用的内存块信息写到交换空间，这样以来，物理内存得到了释放，这块内存就可以用于其它目的，当需要用到原始的内容时，这些信息会被重新从交换空间读入物理内存。
Linux的内存管理采取的是分页存取机制，为了保证物理内存能得到充分的利用，内核会在适当的时候将物理内存中不经常使用的数据块自动交换到虚拟内存中，而将经常使用的信息保留到物理内存。

23、linux的任务调度机制是什么？
现代linux系统中为了协调多个进程的"同时"运行,最基本的手段就是给进程定义优先级.
linux中有两种进程,普通进程和实时进程,实时进程的优先级都高于普通进程

实时进程的调度:
实时系统:就是给定的操作在确定的时间内完成,要达到的目的是时间可控
实时系统分为硬实时和软实时
硬实时要求必须在给定的时间内完成,软实时只是要求尽可能地满足要求的完成时间

内核让可执行状态的实时进程一直执行，直到它完成所需要做的事情，然后睡眠或退出（变为非可执行状态）。
如果有多个实时进程都处于可执行状态，则内核会先满足优先级最高的实时进程对CPU的需要，直到它变为非可执行状态。
如果有相同优先级的多个实时进程处于科执行状态,有两种调度策略可供选择:
...
注意:按照上述方式,系统中只要有实时进程,普通进程是得不到调度的,也就是得不到CPU的使用权.而且，只要高优先级的实时进程一直处于可执行状态，低优先级的实时进程就一直不能得到CPU。后来，内核添加了/proc/sys/kernel/sched_rt_runtime_us和/proc/sys/kernel /sched_rt_period_us两个参数，限定了在以sched_rt_period_us为周期的时间内，实时进程最多只能运行 sched_rt_runtime_us这么多时间。这样就在一直有实时进程处于可执行状态的情况下，给普通进程留了一点点能够得到执行的机会。

普通进程的调度
对于普通进程,调度程序力图让各个处于可执行状态的普通进程公平地分享CPU，从而让用户觉得这些进程是同时高效运行的。
所以关注两个问题:公平性和效率
内核通过维护两个指标来判断当CPU空闲的时候应该调度哪一个进程执行:
1、用户程序设置的优先级（如果未设置，则使用默认值），称为静态优先级。这是进程优先级的基准，在进程执行的过程中往往是不改变的；
2、优先级动态调整后，实际生效的优先级。这个值是可能时时刻刻都在变化的；
为了实现各个进程公平地占用CPU,linux实现公平调度基本上是两种思路：
1、给处于可执行状态的进程分配时间片（按照优先级），用完时间片的进程被放到“过期队列”中。等可执行状态的进程都过期了，再重新分配时间片[前期]；
2、动态调整进程的优先级。随着进程在CPU上运行，其优先级被不断调低，以便其他优先级较低的进程得到运行机会；[后期采用这种]
主要是计算其运行时间和睡眠时间等,基本的思想是不断降低运行过的进程的优先级,以确保低优先级的进程能够得到CPU资源
为了实现系统整体的高效运行,linux(2.6以后)将可执行状态的进程按照优先级顺序被挂在一个红黑树（可以想象成平衡二叉树）中。
...
内核在响应中断的过程中，发现优先级更高的进程所等待的外部资源的变为可用，从而将其唤醒。比如CPU收到网卡中断，内核处理该中断，发现某个socket 可读，于是唤醒正在等待读这个socket的进程；再比如内核在处理时钟中断的过程中，触发了定时器，从而唤醒对应的正在nanosleep系统调用中睡眠的进程；



24、标准库（STL）函数和系统调用的区别？
从用户使用的角度来看两者：
功能上的区别并不是很大，它们都以C函数的形式出现，两者都为应用程序提供服务。
但是从易用性和移植性方面，标准库函数往往好于系统调用，因为系统调用时某个或者某类系统对外提供的接口，而标准库大多是将不同系统上相同功能（接口格式参数可能不同）的系统调用封装成统一格式提供给用户使用的
比如说CRT中的fread在linux系统中封装了read 在windows系统中封装了ReadFile这个API
...
30.Linux中的僵尸进程和孤儿进程
孤儿进程：
顾名思义，就是没有父进程的进程，父进程在创建子进程之后先于子进程结束了。
...
但是如果父进程不对子进程调用wait/waitpid且父进程一直处于活着状态（子进程无法变成孤儿进程）的话，这些信息没人管理，一直像僵尸一样在系统中游荡，所以有了僵尸进程的概念。
...
集合的自反性、对称性和传递性以及反自反性、反对称性
假设集合A，基于A的关系集合为R
自反性：如果a是A中的元素，那么<a,a>是R中的元素
反自反性：如果a是A中的元素，那么<a,a>不是R中的元素
对称性：如果<a,b>是R中的元素，那么<b,a>是R中的元素
反对称性：如果<a,b>，<b,a>是R中的元素，那么a,b相等
传递性：如果<a,b>，<b,c>是R中的元素，那么<a,c>是R中的元素

2、各类排序：大根堆的实现，快排（如何避免最糟糕的状态？），bitmap的运用等等
大根堆（堆必是一棵完全二叉树）的实现：
快排避免最糟糕状态的方法是在选择主元的时候采用一些策略，比如说取三个数的中位数的方法
或者说采用随机的方法取主元，这样能够避免最坏情况的发生
bitmap的使用：


3、hash， 任何一个技术面试官必问（例如为什么一般hashtable的桶数会取一个素数？如何有效避免hash结果值的碰撞）
素数好像就是为了避免碰撞，应该是数学上给出的解决方案
...
注意要区分是TCP的还是HTTP的Keep alive
TCP的KeepAlive和HTTP的Keep-Alive是完全不同的概念

TCP的keepalive：
keepalive，是在TCP中一个可以检测死连接的机制。
keepalive原理很简单，TCP会在空闲了一定时间后发送数据（数据为空的一个包）给对方：
...
HTTP的keep-alive
一个完整的HTTP事务，有链接的建立，请求的发送，响应接收，断开链接这四个过程,早期通过HTTP协议传输的数据以文本为主，一个请求可能就把所有要返回的数据取到，
但是，现在要展现一张完整的页面需要很多个请求才能完成，如图片,JS,CSS等，如果每一个HTTP请求都需要新建并断开一个TCP，这个开销是完全没有必要的。
...
HTTP/1.1之后默认开启Keep-Alive, 在HTTP的头域中增加Connection选项。当设置为Connection:keep-alive表示开启，设置为Connection:close表示关闭。
...
2、Windows API和Linux 系统调用
因为windows系统时闭源的所以操作系统内核提供给外部的接口被windows API又封装了一遍，对外提供windows API供wondows程序员编写程序使用
linux是开源操作系统，所以程序员可以使用到最底层的接口
CRT（C RunTime）是Windows API和linux系统调用上面的一层封装
...
4、c++ template 在编译的时候哪个阶段展开？
应该是在编译的阶段实现的
而不是在预处理阶段（和宏定义和include展开不是一个概念）
只有在首次使用到模板的时候才会去查找模板
比如说vector<int>这样的才会去找vector模板
...
主要是出于安全性的考虑
linux中划分出这两种空间，能够保证核心代码在内核态执行
用户只能在用户层进行操作，这样能够实现多用户
linux通过中断和系统调用作为二者的交互接口


2.多重继承为什么会含有多个虚表指针而不是一个？
主要是在类型转换的时候加快效率
答：多重继承下，因为编译器对一个derived class实现了n-1个虚表，n表示上一层base class的个数，
当然假设每个base class都有至少有一个virtual函数，否则编译器是不会为其添加vptr和vtbl了。所以说有多少个虚表，自然就有多少个指针指向，而不是一个。这样说我不知道合理不合理，
可能面试官要问的点是“为什么需要多个虚表？一个虚表行不行？”
这个属于编译器厂商做的事情，标准并未规范。C++的父亲就做出过这样的一款编译器原型，通过增大vtbl的体积，每个slot上不只有一个指针，还有一个offset，用来调整this指针的指向。
这样做的弊端是：
所有vtbl中的虚函数指针都包含这样一个offset，并且假设不需要调整this指向，调用时还是要做offset的加法操作，尽管offset此时为0。所有vtbl中的虚函数指针都包含这样一个offset，并且假设不需要调整this指向，调用时还是要做offset的加法操作，尽管offset此时为0。
另外，vtbl中每个slot体积的膨胀。这些都是效率问题。
...
4.读文件时系统和硬盘会做哪些工作。
linux中文件读取的主要过程是：
首先用户进程上描述符（task_struct）挂上文件对象（file），其中包含访问的文件的路径名，访问方式函数指针接口等信息。
系统根据路径名和文件名从指定的当前路径（也就是.）或者是根目录（/）中开始查找目录项（dentry）高速缓存
这个目录项高速缓存实际上是一种软件缓存，主要的目的是加速文件系统中文件的查找。
目录项高速缓存中是一个个的目录项信息，这些目录项在系统中首次查找时建立起来，它们是通过hash来进行存储的，通过LRU方式进行更新
目录项中存放了和文件在物理磁盘中保存位置相关的索引节点对象（inode，注意这不是磁盘中的inode）信息。
当系统找到文件对象对应的目录项高速缓存中的目录项（dentry）的时候，取出其中的索引节点（inode）信息。
根据查找的文件所在的文件系统类型找到文件系统的superblock（超级块）和已经查找到的inode（索引节点）就可以确定磁盘中文件的位置
然后可以开展后续的访问工作了，也就是物理上的磁盘操作了。
物理上的磁盘要做的是移臂、寻道和数据传输等等，注意它一次读取是一个扇区的数据。

5.select、poll和epoll。
参见上面

1.谈项目细节，问的比较深，改进什么的，多并发一类处理，总之往大数据上靠，你怎么解决。
2.写两段代码，一个是打印九九乘法表（简单，主要考代码风格吧），
#include <iostream>
#include <cstdio>
#include <cstdlib>
using namespace;
...
3.socket服务端接收代码（这个写的不好，主要考IO阻塞的、问了同步异步）。
同步异步实际上是消息传递的方式
阻塞非阻塞实际上是进程或者线程的工作状态的改变



3.问了文件系统
VFS(虚拟文件系统)
主要是通过一层虚拟文件系统将linux系统中支持的文件系统进行统一管理
包括文件系统的安装和使用
-------------------------
      上层用户使用
-------------------------
...
-------------------------
           VFS（文件系统的管理者）
-------------------------
挂载到VFS的各实际文件系统（rootfs、proc、ext2、sockfs）
-------------------------
         物理磁盘
-------------------------
涉及到的概念：
...
注册过程实际上将表示各实际文件系统的 struct file_system_type 数据结构的实例化，然后形成一个链表，
内核中用一个名为 file_systems 的全局变量来指向该链表的表头。
